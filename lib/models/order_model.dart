...
class OrderModel {
  ...
  factory OrderModel.fromJson(Map<String,dynamic> jsonData, String idOrder){
    return OrderModel(
        ...
        productsList: List<OrderProductModel>.from(
            jsonData["productsList"].map((product) => OrderProductModel.fromJson(product))
        ),
    );
  }

  static List<OrderModel> fromJsonList(List<QueryDocumentSnapshot> ordersList) {
    return ordersList.map((order) => OrderModel.fromJson(order.data() as Map<String, dynamic>, order.id)).toList();
  }
